...
class mp3_tree {
	...
	function options_array() {
		return array( "reset_db", "no_html_output", "js_output", "in_frame", "scan_method" );
	}
	...
	function html_output( $html ) {
		if ( !$this->options["NO_HTML_OUTPUT"] ) {
			echo $html;
			if ( $this->options["IN_FRAME"] ) echo "<br />\n";
			ob_flush();
			flush();
		}
	}
	...
	function process_tree() {
		...
		if ( $this->import_path ) $query .= " where full_path LIKE '".$this->import_path."%'";
		$this->db->query( $query );
	...
	function count_files( $from, $parentID = 0 ) {
		...
		return array( "num_files"=>$num_files, "num_dirs"=>$num_dirs );
	}

	function convert_tags( $fileinfo ) {
		
		$tag_array = array( "artist", "title", "genre", "track", "album", "year", "year", "comment" );
	...
	function cut_full_path( $path ) {
		if ( $path==MP3_PATH ) return "";
		else return substr( $path, ( strlen( $path )-strlen( MP3_PATH."/" ) )*-1 );
	}
	...
	function get_dirID( $from, $parentID ) {
		...
		return array( "new"=>$new, "dirID"=>$dirID );
	}
	...
	function update_song( $fileinfo, $id, $full_path ) {
		...
		$albumID = $this->get_tagID( "album", $fileinfo["filename"], $full_path );
		
		$artistID = $this->get_tagID( "artist", $fileinfo["artist"] );
		
		$genreID = $this->get_tagID( "genre", $fileinfo["genre"] );
		...
		$query = "UPDATE
							...
							SET
								...
								album = '".addslashes( $fileinfo["filename"] )."',
	...
	function finish() {
		...
		if ( $this->options["SCAN_METHOD"]=="ALL" ) {
			...
			if ( $this->import_path ) $query .= " AND full_path LIKE '".$this->import_path."%'";
		...
		$query = "UPDATE genre SET num_files = 0";
		$result = $this->db->query( $query );
		$query = "UPDATE genre SET genre.num_files = (SELECT count(*) FROM song WHERE genre.ID = song.genreID group by genre.ID)";
		$result = $this->db->query( $query );
		$query = "DELETE from genre WHERE num_files = 0";
		$result = $this->db->query( $query );
