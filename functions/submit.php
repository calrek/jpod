...
function is_unique($tab, $field) {
	...
	if ($_POST["ID"]) $query.= " AND ID != '" . $_POST["ID"] . "'";
...
if (!$_POST["tab"]) $tab = $_POST["submit_type"];
else $tab = $_POST["tab"];
...
switch ($_POST["action"]) {
	case "save":
		...
		switch ($_POST["submit_type"]) {
			case "user":
				if (!$_SESSION["permission"]["useradmin"]) json_no_permission();
				...
				if ($_POST["ID"] != $guestID AND $_POST["ID"] != $adminID) {
					if (!is_unique($tab, "name")) $result["errors"]["name"] = $lang["username_already_exists"];
					$field_array["name"] = array();
				}

				if (($_POST["password"] OR $_POST["password2"]) AND $_POST["password"] != $_POST["password2"]) $result["errors"]["password"] = $lang["passwords_dont_match"];
				elseif ((!$_POST["password"] OR !$_POST["password2"]) AND !$_POST["ID"]) $result["errors"]["password"] = $lang["password_missing"];
			...
			if (sizeof($result["errors"])) $result["success"] = false;
			else {
				reset($field_array);
				...
				if ($affected_rows) $result["success"] = true;
