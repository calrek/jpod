...
if ($node == - 1) $node = 0;
/*{
	$query = "SELECT ID FROM path WHERE parentID = 0 ORDER BY name";
	$result = $db->query($query);
	$node = $db->result($result,0,0);
}*/
...
while ($row = $db->fetch_array($result)) {
	...
	if ($row["num_dirs"]) $is_leaf = 0;
	else $is_leaf = 1;
	
	if ($row["parentID"]) $my_root = 0;
	else {
		$my_root = 1;
		$row["name"] = "MP3";
	}
	
	if ($num_files_total) $nodes[] = array(
		'name' => $row["name"],
