...
if (!$_SESSION["permission"]["edit_own_playlists"] and !$_SESSION["permission"]["edit_all_playlists"]) die($lang["no_permission"]);
	...
	case "edit":
		if ($_POST["name"]) {
			...
			if ($db->num_rows($result)) {
				...
				if (!$_SESSION["permission"]["edit_all_playlists"]) $query.= " AND userID = '" . $_SESSION["userID"] . "'";
				$result = $db->query($query);
				if ($db->affected_rows($result)) $success = true;
				else $success = "false";
			}
		}
		break;
