...
if (!$_REQUEST["text"]) $text = "name";
else $text = $_REQUEST["text"];

if (!$_REQUEST["sort"]) $sort = $text;
else $sort = $_REQUEST["sort"];

if (!$_REQUEST["tab"]) $tab = $_REQUEST["type"];
else $tab = $_REQUEST["tab"];
...
switch ($tab) {
	case "playlist":
		if (!$_SESSION["permission"]["see_all_playlists"]) {
			$query = "SELECT
								playlist.ID,
								playlist.name as text_to_display,
			...
			$query = "SELECT
							...
							LEFT JOIN playlist_song ON (playlist.ID = playlist_song.playlistID)
							GROUP BY playlist.ID
							ORDER BY if(playlist.userID=" . $_SESSION["userID"] . ",0,1),playlist.name
							";
		}
		break;
...
if ($_REQUEST["addnobody"]) {
	if ($_REQUEST["nobody_text"]) $nobody_text = $_REQUEST["nobody_text"];
	else $nobody_text = lang("nobody");
	$pointer = sizeof($data_array);
...
if ($query) {
	...
	while ($row = $db->fetch_array($result)) {
		...
		if ($_REQUEST["tab"] == "playlist") {
			if (!$_SESSION["permission"]["see_all_playlists"]) $data_array[$pointer]["tooltip"] = $row["num"] . " " . lang("files");
			else $data_array[$pointer]["tooltip"] = $row["num"] . " " . lang("files") . "<br />" . lang("created_by") . " <b>" . $row["user"] . "</b>";

			echo "/* " . $row["userID"] . " = " . $_SESSION["userID"] . " */";
			if ($row["userID"] == $_SESSION["userID"]) $class_add = "";
			else $class_add = " other-playlist";
