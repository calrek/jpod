...
function album_list() {
	...
	var title = '<?php echo lang("album_grid_title"); ?>';
	var alias = "album";
	...
	var cm = new Ext.grid.ColumnModel([{
		dataIndex: 'name',
		header: '<?php echo lang("album"); ?>',
		renderer: renderAlbum,
		sortable: true
	}, {
		dataIndex: 'artist',
		header: '<?php echo lang("artist"); ?>',
	...
	function album_name(value, p, record) {
		...
		if (record.get('cover_resized') == 1) 
		
			var path = record.get('cover') + "/s72/";
	...
	var grid = new Ext.grid.GridPanel({
		...
		enableColLock: false,
		loadMask: true,
		plugins: [new Ext.ux.grid.GridViewMenuPlugin, new Ext.ux.grid.Search({
			iconCls: 'icon-zoom',
			searchText: '',
			readonlyIndexes: [],
			disableIndexes: [],
		...
		tbar: new Ext.Toolbar({
			items: [{
				tooltip: '<?php echo lang("change_view", 1); ?>',
				iconCls: 'icon-view',
				menu: [{
						group: 'view_album',
						checkHandler: changeView,
						checked: tpl_name == "giantCover",
						text: '<?php echo lang("big_cover", 1); ?>',
						type: 'giant'
					}, {
						group: 'view_album',
						checkHandler: changeView,
						checked: tpl_name == "mediumCover",
						text: '<?php echo lang("medium_cover", 1); ?>',
						type: 'medium'
					}, {
						group: 'view_album',
						checkHandler: changeView,
						checked: tpl_name == "tinyCover",
						text: '<?php echo lang("small_cover", 1); ?>',
						type: 'tiny'
					}, {
						group: 'view_album',
						checkHandler: changeView,
						checked: tpl_name == "",
						text: '<?php echo lang("list", 1); ?>',
						type: 'list'
					}
				]
			}]
		...
		bbar: new Ext.PagingToolbar({
			store: ds,
			pageSize: <?php echo ENTRIES_IN_ALBUM_LIST; ?> , displayInfo: false,
			listeners: {
  ...
  scope : this
	...
	ds.load({
		params: {
			start: 0,
			limit: <?php echo ENTRIES_IN_ALBUM_LIST; ?>
		}
	});
