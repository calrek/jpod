...
function artist_list() {
	...
	var title = '<?php echo lang("artist_grid_title", 1); ?>';
	var alias = "artist";
	...
	var expander = new Ext.grid.RowExpander({
		remoteDataMethod: function(record, index) {
			...
			if (!record.get("loaded")) {
				Ext.get("filter_panelID").mask('<?php echo lang("loading_albums", 1); ?>');
				Ext.Ajax.request({
	...
	var grid = new Ext.grid.GridPanel({
		...
		bbar: new Ext.PagingToolbar({
			store: ds,
			pageSize: <?php echo ENTRIES_IN_ARTIST_LIST; ?> ,
			displayInfo: false
		})
	});
	...
	ds.load({
		params: {
			start: 0,
			limit: <?php echo ENTRIES_IN_ARTIST_LIST; ?>
		}
	});
