...
function play_list() {
    ...
    var actions = new Ext.ux.grid.RowActions({
        ...
        getData: function(value, cell, record, row, col, store) {
            ...
            else echo "0"; ?> || pl.getValue() <= 0 || (userID == <?php echo $_SESSION["userID"]; ?> && <?php
        ...
        actions: [{
            iconCls: 'icon-delete',
            tooltip: '<?php echo lang("delete", 1); ?>',
    ...
    var cm = new Ext.grid.ColumnModel([{
        ...
        sortable: true,
        dataIndex: 'location',
        header: '<?php echo lang("location", 1); ?>'
    }, {
        hidden: true,
        sortable: true,
        dataIndex: 'filemtime',
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        new Ext.form.ComboBox({
            ...
            store: new Ext.data.JsonStore({
                ...
                baseParams: {
                    ...
                    no_assign: 1,
                    addnobody: 1,
                    nobody_text: '<?php echo lang("none", 1); ?>'
                },
            ...
            mode: 'local',
            triggerAction: 'all',
            emptyText: '<?php echo lang("choose_playlist", 1); ?>',
            selectOnFocus: true,
            width: 160,
            listWidth: 230,
        ...
        if ($_SESSION["permission"]["edit_own_playlists"] OR $_SESSION["permission"]["edit_all_playlists"]) { ?> , {
                iconCls: 'icon-playlist-menu',
                // <-- icon
                menu: [{
                    id: "playlist_add_buttonID",
                    iconCls: 'icon-add',
                    text: '<?php echo lang("create_playlist", 1); ?>',
                    handler: create_playlist
                }, {
                    disabled: true,
                    id: "playlist_edit_buttonID",
                    iconCls: 'icon-edit',
                    text: '<?php echo lang("edit_playlist", 1); ?>',
                    handler: edit_playlist
                }, {
                    disabled: true,
                    id: "playlist_delete_buttonID",
                    iconCls: 'icon-delete',
                    text: '<?php echo lang("delete_playlist", 1); ?>',
                    handler: delete_playlist
                }, {
                    disabled: true,
                    id: "playlist_clear_buttonID",
                    iconCls: 'icon-clear-playlist',
                    text: '<?php echo lang("clear_playlist", 1); ?>',
    ...
    function clear_playlist() {
        Ext.Msg.show({
            ...
            msg: '<?php echo lang("clear_playlist_confirm", 1); ?>',
    ...
    function delete_playlist() {
        Ext.Msg.show({
            title: '<?php echo lang("warning", 1); ?>',
            msg: '<?php echo lang("delete_playlist_confirm", 1); ?>',
            buttons: Ext.Msg.YESNO,
            buttonText: Ext.MessageBox.buttonText.yes = '<?php echo lang("yes", 1); ?>',
            buttonText: Ext.MessageBox.buttonText.no = '<?php echo lang("no", 1); ?>',
            icon: Ext.MessageBox.QUESTION,
                    ...
                    Ext.Ajax.request({
                        ...
                        failure: function(form, action) {
                            Ext.Msg.show({
                                ...
                                msg: '<?php echo lang("error_deleting_playlist", 1); ?>',
                                minWidth: 200,
                                modal: true,
                                icon: Ext.Msg.ERROR,
                                buttons: Ext.Msg.OK
    ...
    function edit_playlist() {
        Ext.MessageBox.prompt('<?php echo lang("edit_playlist", 1); ?>', '<?php echo lang("enter_new_playlist_name", 1) ?>:', function(btn, text) {
            if (btn == "ok") {
                if (!text) {
                    Ext.Msg.show({
                        title: '<?php echo lang("error", 1); ?>',
                        msg: '<?php echo lang("error_no_playlist_name", 1); ?>',
                        minWidth: 200,
                        modal: true,
                        icon: Ext.Msg.ERROR,
                        buttons: Ext.Msg.OK
    ...
    function create_playlist() {
        Ext.MessageBox.prompt('<?php echo lang("new_playlist", 1); ?>', '<?php echo lang("enter_new_playlist_name", 1); ?>:', function(btn, text) {
            if (btn == "ok") {
                if (!text) {
                    Ext.Msg.show({
                        title: '<?php echo lang("error", 1); ?>',
