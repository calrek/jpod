...
function build_query( $type ) {
	...
	case "album":
		...
		$query["fields"] = "album.ID,
			...
			if(album.cover_resized='0',
			CONCAT('',album.cover,'s72/'),
			'img/cover_no_album.gif') AS cover,
	...
	case "artist":
		...
		$query["fields"] = "artist.ID,
			CONCAT(if(artist.name!='',artist.name,'[" . $lang["unknown"] . "]'),' (',CAST(artist.num_files AS CHAR),')') AS name ";
		$query["from"] = "artist";
		$query["where"] = "
			AND num_files > 0";
		break;
	...
	case "genre":
		...
		$query["fields"] = "genre.ID,
			CONCAT(if(genre.name!='',genre.name,'[" . $lang["unknown"] . "]'),' (',CAST(genre.num_files AS CHAR),')') AS name ";
		$query["from"] = "genre";
		$query["where"] = "
			AND num_files > 0";
		break;
	...
	case "playlist":
		...
		if ( $_REQUEST["playlistID"] >= 0 ) {
				...
				song.genre,
				    ( SELECT rating
				     ...
				     WHERE song.ID = song_rating.songID
				         AND song_rating.userID = " . $_SESSION["userID"] . " ) AS rating,
				song.comment,
				song.year,
				song.track,
				song.filesize,
			...
			$query["from"] = "song";
			 ...
			 switch 
				($_REQUEST["playlistID"]) { 
					CASE -3: $query["where"] = " AND last_played != 0 ORDER BY " . $orderby . " LIMIT " . ENTRIES_IN_TOP_PLAYLIST;
			...
			AND
			    ( SELECT rating
			     FROM song_rating
			     WHERE song.ID = song_rating.songID
			         AND song_rating.userID = " . $_SESSION["userID"] . " ) = 5
			ORDER BY " . $orderby . " LIMIT " . ENTRIES_IN_TOP_PLAYLIST;
			...
			AND
			    ( SELECT rating
			     FROM song_rating
			     WHERE song.ID = song_rating.songID
			         AND song_rating.userID = " . $_SESSION["userID"] . " ) >= 4
			ORDER BY " . $orderby . " LIMIT " . ENTRIES_IN_TOP_PLAYLIST;
			...
			AND
			    ( SELECT rating
			     FROM song_rating
			     WHERE song.ID = song_rating.songID
			         AND song_rating.userID = " . $_SESSION["userID"] . " ) >= 3
			ORDER BY " . $orderby . " LIMIT " . ENTRIES_IN_TOP_PLAYLIST;

			 break;

			 DEFAULT: $query["where"] = " ORDER BY " . $orderby . " LIMIT " . ENTRIES_IN_TOP_PLAYLIST;
		...
		if ( $_REQUEST["genreID"] ) {
			$query["where"].= " AND genreID = '" . $_REQUEST["genreID"] . "'";
		}
...
function check_cache( $query ) {
	...
	if ( !$query["no_cache"] ) {
		...
		if ( $db->num_rows( $result ) ) {
			...
			if ( !$query["data_table"] ) $query["data_table"] = $query["from"];

			$query["data"] = "SELECT " . $query["fields"] . " FROM " . "cache_entry," . $query["from"] . " WHERE cache_entry.rowID = " . $query["data_table"] . ".ID AND cache_entry.cacheID = " . $query["cacheID"] . finalize_query( $query["secondary_sort"], 1 );
		} else {
			// echo "/* no */";
...
function build_where( $filter_name = array() ) {
	...
	if ( is_array( $filter ) ) {
		...
		for ( $i = 0; $i < count( $filter ); $i++ ) {
			...
			if ( array_key_exists( $field, $filter_name ) ) $field = $filter_name[$field];
	...
	if ( $_REQUEST["full_text_search"] ) {
		$where.= " AND MATCH(title,artist,album,genre) AGAINST (\"" . $_REQUEST["full_text_search"] . "\")";
	}
	return $where;
}
function finalize_query( $secondary_sort, $no_limit = 0 ) {
	...
	if ( $sort == "track" ) $sort = "LPAD(track,10,'0')";
	...
	if ( $sort != "" ) {
		if ( $sort == "artist" and $_REQUEST["type"] == "album" ) $end.= " ORDER BY if(artistID!=0,0,1),artist " . $dir . ",name " . $dir;
		else $end.= " ORDER BY " . $sort . " " . $dir;

		if ( $_SESSION[$_REQUEST["type"]]["secondary_sort"] ) $end.= ", " . $_SESSION[$_REQUEST["type"]]["secondary_sort"] . " " . $_SESSION[$_REQUEST["type"]]["secondary_sort_dir"];
	}
	if ( $count and !$no_limit ) $end.= " LIMIT " . $start . "," . $count;
