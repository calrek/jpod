function playlist_export_form() {
	...
	myDataStore = new Ext.data.Store({
		url: 'functions/json.data.php?tab=playlist&no_assign=1',
		reader: jsonReader,
		autoLoad: true,
		remoteSort: false,
		listeners: {
...
function user_form(id_to_load) {
	...
	var stream_option_data = [
		['stream', '<?php echo lang("stream_files", 1); ?>'],
		['direct', '<?php echo lang("dont_stream_files", 1); ?>']
	];
	...
	var fs = new Ext.FormPanel({
		...
		new_function: function() {
			Ext.Msg.show({
				title: '<?php echo lang("warning", 1); ?>',
				msg: '<?php echo lang("warning_changes_get_lost", 1); ?>',
				buttons: Ext.Msg.YESNO,
				buttonText: Ext.MessageBox.buttonText.yes = '<?php echo lang("yes", 1); ?>',
				buttonText: Ext.MessageBox.buttonText.no = '<?php echo lang("no", 1); ?>',
		...
		load_function: function(value) {
			...
			if (value > 0) {
				...
				this.getForm().load({
					url: 'functions/get_form_data.php?tab=' + form_name + '&id=' + value,
					success: function(f, a) {
						var checkboxes = Ext.getCmp(form_name + "permissions_checkboxgroup").items;
						for (i = 0; i < checkboxes.items.length; i++) {
							if (a.result.data[checkboxes.items[i].name])
								Ext.getCmp(checkboxes.items[i].id).setValue(true);
							else
								Ext.getCmp(checkboxes.items[i].id).setValue(false);
						...
						if (Ext.getCmp(form_name + 'navigationID').getValue()) {
							...
							if (Ext.getCmp(form_name + 'formID').getForm().reader.jsonData.rows[0].type == "guest" || Ext.getCmp(form_name + 'formID').getForm().reader.jsonData.rows[0].type == "admin")
								Ext.getCmp(form_name + 'user_name').disable();
							else
								Ext.getCmp(form_name + 'user_name').enable();
						} else
		...
		items: [
			new Ext.form.ComboBox({
				...
				fieldLabel: '<?php echo lang("user", 1); ?>',
				hiddenName: 'ID',
				...
				mode: 'local',
				triggerAction: 'all',
				emptyText: '<?php echo lang("create_new_user", 1); ?>',
				selectOnFocus: true,
				width: 250,
			...
			new Ext.Panel({
				...
				items: [{
					...
					defaultType: 'textfield',
					items: [{
							fieldLabel: '<?php echo lang("name", 1); ?>',
							id: form_name + 'user_name',
							name: 'name',
							allowBlank: false,
							invalidText: '<?php echo lang("please_enter_username", 1); ?>',
							width: 190
						}, {
							fieldLabel: '<?php echo lang("password", 1); ?>',
							id: form_name + 'user_password',
							name: 'password',
							allowBlank: true,
							invalidText: '<?php echo lang("please_enter_password", 1); ?>',
							width: 190,
							inputType: 'password'
						}, {
							fieldLabel: '<?php echo lang("repeat_password", 1); ?>',
							id: form_name + 'user_password2',
							name: 'password2',
							allowBlank: true,
							invalidText: '<?php echo lang("please_repeat_password", 1); ?>',
							width: 190,
							inputType: 'password',
							vtype: 'password',
						...
						new Ext.form.ComboBox({
							...
							fieldLabel: '<?php echo lang("access_option", 1); ?>',
							valueField: 'value',
							displayField: 'text',
							typeAhead: true,
							mode: 'local',
							...
							xtype: 'checkboxgroup',
							fieldLabel: '<?php echo lang("permissions", 1); ?>',
		...
		handler: function() {
			if (!Ext.getCmp(form_name + 'navigationID').getValue()) {
				...
				if (Ext.getCmp(form_name + 'user_password').getValue() != Ext.getCmp(form_name + 'user_password2').getValue()) {
					Ext.getCmp(form_name + 'user_password2').markInvalid("<?php echo lang('passwords_dont_match', 1); ?>");
					return false;
				}
			}
			submitting = true;
			...
			fs.getForm().submit({
				...
				waitMsg: '<?php echo lang("saving", 1); ?>',
				success: function(form, action) {
					enable_disable_buttons(form_name, true);
					submitting = false;
					Ext.Msg.show({
						title: '<?php echo lang("save", 1); ?>',
						msg: "<?php echo lang('saving_successfull', 1); ?>",
						minWidth: 200,
						modal: true,
						icon: Ext.Msg.INFO,
						buttons: Ext.Msg.OK
				...
				failure: function(form, action) {
					...
					Ext.Msg.show({
						...
						msg: "<?php echo lang('error_saving', 1); ?>",
						minWidth: 200,
						modal: true,
						icon: Ext.Msg.ERROR,
						buttons: Ext.Msg.OK
	...
	fs.addButton({
		...
		function() {
			Ext.Msg.show({
				title: '<?php echo lang("warning", 1); ?>',
				msg: '<?php echo lang("confirm_user_delete", 1); ?>',
				buttons: Ext.Msg.YESNO,
				...
				fn: function(btn) {
					if (btn == "yes") {
						...
						fs.getForm().submit({
							...
							success: function(form, action) {
								Ext.Msg.show({
									title: '<?php echo lang("delete", 1); ?>',
									msg: '<?php echo lang("delete_succesfull", 1); ?>',
									minWidth: 200,
									modal: true,
									icon: Ext.Msg.INFO,
									buttons: Ext.Msg.OK
							...
							failure: function(form, action) {
								...
								Ext.Msg.show({
									title: '<?php echo lang("error", 1); ?>',
									msg: '<?php echo lang("error_deleting_user", 1); ?>',
									minWidth: 200,
									modal: true,
									icon: Ext.Msg.ERROR,
									buttons: Ext.Msg.OK
