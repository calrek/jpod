function player() {
	...
	this.save_playlist = function() {
		...
		if (Ext.getCmp("playlist_selectionID").getValue() > 0) {
			...
			Ext.getCmp("playlistID").getEl().mask("<?php echo lang('saving', 1); ?>");
			Ext.Ajax.request({
				...
				failure: function(form, action) {
					Ext.Msg.show({
						...
						msg: '<?php echo lang("error_saving_playlist", 1); ?>',
						minWidth: 200,
						modal: true,
						icon: Ext.Msg.ERROR,
						buttons: Ext.Msg.OK
	...
	this.play_song = function() {
		if (this.inited == 0) {
			...
			var genre = htmlspecialchars_decode(data.genre);
			...
			this.stop_playlist = false;
				...
				if ($_SESSION["access_option"] == "direct") { ?>
						url = data.location;
					Ext.Ajax.request({
						url: 'functions/num_plays.php',
						params: {
							ID: data.ID
						}
					}); <?php
				} else { ?> url = "functions/get_stream.php?ID=" + data.ID; <?php
				} ?>

			if (data.artistID && artist) document.getElementById("info_artist").innerHTML = "<a href=\"javascript: void(0)\" onClick=\"filter_setting('artist'," + data.artistID + ");\">" + artist + "</a>";
			else document.getElementById("info_artist").innerHTML = "-";

			if (data.genreID && genre) document.getElementById("info_genre").innerHTML = "<a href=\"javascript: void(0)\" onClick=\"filter_setting('genre'," + data.genreID + ");\">" + genre + "</a>";
			else document.getElementById("info_genre").innerHTML = "-";
			...
			if (bit_rate) document.getElementById("info_bitrate").innerHTML = bit_rate;
			...
			else document.title = "<?php echo $lang['unknown_song']; ?>";
			...
			//document.getElementById("info_cover").src = "get_cover.php?albumID=" + data.albumID + "&artist=" + encodeURIComponent(artist) + "&album=" + encodeURIComponent(album);
			document.getElementById("info_cover").src = "functions/get_cover.php?ID=" + data.ID;
