...
function open_youtube_url(url) {
	Ext.getCmp("youtube_windowID").load({
		url: "youtube_url.php",
		callback: function(el, success, response, options) {
			youtube_cache_page2 = response.responseText;
		...
		listeners: {
			itemclick: function(item) {
				...
				switch (item.myid) {
						...
						youtube_window = new Ext.Window({
							...
							tbar: [{
								...
								handler: function() {
									...
									Ext.getCmp("youtube_windowID").body.update(youtube_cache_page1);
								...
								handler: function() {
									Ext.getCmp("youtube_back").enable();
									Ext.getCmp("youtube_windowID").body.update(youtube_cache_page2);
									Ext.getCmp("youtube_forward").disable();
								}
							}],
							id: 'youtube_windowID',
					...
					case 'lyrics-entry':
						...
						lyrics_window = new Ext.Window({
							...
							autoLoad: {
								...
								params: 'action=getSong&title=' + encodeURIComponent(data.title) + "&artist=" + encodeURIComponent(data.artist)
							},
							plain: false
						});
						lyrics_window.show();
					...
					case 'download-entry':
						window.open("functions/get_download.php?ID=" + data.ID);
						break;
				}
			}
		}
...
function filter_setting(from, value) {
	...
	if (from == "genre") {
		if (value) store.baseParams.genreID = value;
		else store.baseParams.genreID = 0;
	} else store.baseParams.genreID = 0;
...
function renderGenre(value, p, record) {
	if (record.get('genreID') != 0) return String.format('<a href="javascript: void(0)" onClick="filter_setting(\'genre\',{0});">{1}</a>', record.get('genreID'), value);
	else return value;
}
...
function my_about() {
	Ext.MessageBox.show({
		title: '<?php echo lang("about_title", 1); ?>',
		msg: '<?php echo lang("about_text", 1); ?>',
...
var Utf8 = {
			...
			player_window = new Ext.Window({
				...
				listeners: {
					...
					beforeclose: function(p) {
						Ext.getCmp("top_northID").show();
						if (!Ext.getCmp("center_pangelID").isVisible()) {
							Ext.getCmp("center_pangelID").expand(false);
						}
						var playing_info = document.getElementById("playing_info_content");
						var parent_node = playing_info.parentNode;
						...
						Ext.getCmp("center_pangelID").doLayout();
					}
				}
			});
			player_window.show();
			if (player_obj.undock_button) player_obj.undock_button.hide();
			var parent = document.getElementById("playing_info_content");
	...
	function clear_cache() {
		Ext.Msg.show({
			...
			msg: '<?php echo lang("clear_cache_confirm", 1); ?>',
			...
			fn: function(btn) {
				if (btn == "yes") {
						Ext.Ajax.request({
							url: 'functions/clear_cache.php',
							params: {},
							waitMsg: '<?php echo lang("deleting", 1); ?>',
							...
							failure: function(form, action) {
								Ext.Msg.show({
									...
									msg: '<?php echo lang("error_clearing_cache", 1); ?>',
									minWidth: 200,
									modal: true,
									icon: Ext.Msg.ERROR,
									buttons: Ext.Msg.OK
